...
async function selectVariant(page: Page, color: string, size: string) {
  await page.locator('.color-btn', { hasText: color }).first().click()
...
async function addToCartFromProduct(page: Page, slug: string, productId: string, color: string, size: string, qty: number) {
...
test('out-of-stock existing variant is shown disabled (not hidden)', async ({ page }) => {
  ...
  await page.waitForTimeout(300)
  await page.locator('.color-btn', { hasText: 'Black' }).first().click()
  const xl = page.locator('.size-btn', { hasText: 'XL' }).first()
  await expect(xl).toHaveClass(/size-btn--disabled/)
...
test('changing color resets an invalid size selection', async ({ page }) => {
  ...
  await expect(page.locator('.size-btn--active')).toContainText('XXL')
  await page.locator('.color-btn', { hasText: 'White' }).first().click()
...
test('cart keeps different variants as separate lines', async ({ page }) => {
  ...
  await addToCartFromProduct(page, c.slug, productId, 'White', 'L', 1)
...
// never passes through another cancelled state (cancel → deliver → cancel).
...
test('cancel → deliver → cancel restores stock exactly once (no double restore)', async ({ page }) => {
  ...
  // would treat DELIVERED → CANCELLED as a brand-new cancellation.
