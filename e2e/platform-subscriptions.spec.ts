...
test('platform presents subscriptions, lifetime launch offer, and enterprise separately', async ({ page }) => {
  ...
  await expect(page.locator('.plan-grid').first().locator('.plan-pricing-card')).toHaveCount(4)
  await expect(page.getByText('BUSINESS', { exact: true })).toBeVisible()
  await expect(page.getByText('باقة تاريخية محفوظة للتوافق مع الاشتراكات القائمة')).toBeVisible()
