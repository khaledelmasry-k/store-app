...
async function main() {
  ...
  await createPlan('plan-free', 'FREE', 0, 50, 50, 'باقة تاريخية محفوظة للتوافق مع التجارب القائمة قبل إطلاق الباقات المدفوعة', ['متجر إلكتروني', 'إدارة المنتجات', 'الطلبات', 'العملاء', 'لوحة تحكم'], { billingModel: 'subscription', oneTimePrice: 0, priceYearly: 0, trialDays: 30, landingPagesLimit: 0, salesLinksLimit: 0, staffLimit: 1, storageLimit: 200, slug: 'free', sortOrder: 0, active: false, isPurchasable: false, archived: true, unlimitedProducts: false, unlimitedSalesLinks: false, analytics: true })
  await createPlan('plan-basic', 'BASIC', 149, 100, 100, 'الباقة الأساسية للمتاجر الجديدة', ['متجر إلكتروني', 'إدارة المنتجات', 'الطلبات', 'العملاء', 'لوحة تحكم'], { priceYearly: 1490, trialDays: 3, landingPagesLimit: 0, salesLinksLimit: 3, staffLimit: 2, storageLimit: 512, slug: 'basic', sortOrder: 1, unlimitedProducts: false, unlimitedSalesLinks: false, analytics: true })
  await createPlan('plan-starter', 'STARTER', 249, 500, 300, 'للمتاجر التي بدأت البيع وتحتاج أدوات تسويق أساسية', ['كل مزايا Basic', 'كوبونات', 'روابط بيع', 'صفحة هبوط واحدة', 'تخصيص المتجر', 'تنبيهات المخزون'], { priceYearly: 2490, trialDays: 3, launchPrice: 0, launchEnabled: false, landingPagesLimit: 1, salesLinksLimit: 10, staffLimit: 3, storageLimit: 1024, slug: 'starter', sortOrder: 2, unlimitedProducts: false, unlimitedSalesLinks: false, variantInventory: true, coupons: true, analytics: true })
  await createPlan('plan-growth', 'GROWTH', 399, 2000, 1500, 'للمتاجر النامية التي تحتاج أدوات تشغيل أوسع', ['كل مزايا Starter', 'تسعير بالكمية', 'حدود تشغيل أعلى', 'صفحات هبوط متعددة', 'روابط بيع موسعة'], { priceYearly: 3990, trialDays: 3, launchPrice: 0, launchEnabled: false, landingPagesLimit: 5, salesLinksLimit: 50, staffLimit: 10, storageLimit: 5120, slug: 'growth', isPopular: true, sortOrder: 3, unlimitedProducts: false, unlimitedSalesLinks: false, quantityPricing: true, variantInventory: true, coupons: true, analytics: true })
  ...
  await createPlan('plan-pro', 'PRO', 649, 0, 10000, 'للمتاجر الكبيرة التي تحتاج حدوداً أعلى', ['كل مزايا Growth', 'أعلى حدود استخدام', 'منتجات غير محدودة', 'روابط بيع غير محدودة'], { priceYearly: 6490, trialDays: 3, launchPrice: 0, launchEnabled: false, landingPagesLimit: 20, salesLinksLimit: 0, staffLimit: 50, storageLimit: 20480, slug: 'pro', sortOrder: 4, unlimitedProducts: true, unlimitedSalesLinks: true, quantityPricing: true, variantInventory: true, coupons: true, analytics: true })
  ...
  await createSubscription('store-b', 'plan-starter', 'active', {
    ...
    approvedBy: adminUid,
     ordersUsed: 276,
     periodNumber: 2,
     normalPriceSnapshot: 249,
     launchPriceSnapshot: 249,
     yearlyPriceSnapshot: 2490,
     launchUsed: false,
   })
  ...
  await createSubscription('store-d', 'plan-pro', 'expired', {
    ...
    normalPriceSnapshot: 649,
    launchPriceSnapshot: 649,
    yearlyPriceSnapshot: 6490,
    ordersUsed: 40,
  })
