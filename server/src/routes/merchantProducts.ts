...
function storeScope(req: Request): { storeId?: string } | {} {
  return req.storeId ? { storeId: req.storeId } : {};
}

router.get("/low-stock", async (req: Request, res: Response) => {
  const where: any = storeScope(req);
  where.active = true;
...
router.get("/:id", async (req: Request<{ id: string }>, res: Response) => {
  const where: any = { id: String(req.params.id), ...storeScope(req) };
...
router.put("/:id", requirePermission("products", "edit"), async (req: Request<{ id: string }>, res: Response) => {
  const where: any = { id: String(req.params.id), ...storeScope(req) };
...
router.post("/:id/duplicate", requirePermission("products", "create"), async (req: Request<{ id: string }>, res: Response) => {
  const where: any = { id: String(req.params.id), ...storeScope(req) };
