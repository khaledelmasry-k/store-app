...
async function merchantProviderEligibility(provider: any, config: any, store: any, shipmentVolume: number): Promise<any> {
  ...
  const agreement = provider?._commercialAgreement
  const agreementActive = agreement?.status === 'active' && (!agreement.effectiveFrom || (toMillis(agreement.effectiveFrom) ?? Infinity) <= Date.now()) && (!agreement.effectiveTo || (toMillis(agreement.effectiveTo) ?? -Infinity) >= Date.now())
  const providerReady = provider?.status === 'active'
    && provider?.partnership?.status === 'active'
    && provider?.eligibilityConfig?.enabled !== false
  ...
  if (minimum > 0 && effective < minimum) reasons.push({ code: 'MINIMUM_VOLUME_NOT_MET', message: `هذه الشركة تبدأ من ${minimum} شحنة شهريًا، وحجم متجرك الحالي ${shipmentVolume} شحنة.` })
...
export const getMerchantShippingProviders = onCall(async (request: CallableRequest<{ storeId?: string }>) => {
  ...
  const [providersSnap, configsSnap, credentialsSnap, settingsSnap, storeSnap, shipmentsSnap] = await Promise.all([
    ...
    db.doc(`stores/${storeId}`).get(),
    db.collection('shipments').where('storeId', '==', storeId).where('createdAt', '>=', Timestamp.fromMillis(Date.now() - 30 * 24 * 60 * 60 * 1000)).get(),
  ])
...
export const saveStoreShippingProvider = onCall(async (request: CallableRequest<any>) => {
  ...
  if (config.enabled) {
    const storeSnap = await db.doc(`stores/${storeId}`).get()
    const shipmentsSnap = await db.collection('shipments').where('storeId', '==', storeId).where('createdAt', '>=', Timestamp.fromMillis(Date.now() - 30 * 24 * 60 * 60 * 1000)).get()
    const agreementSnap = await db.doc(`shippingProviderCommercialAgreements/${providerId}`).get()
    const eligibility = await merchantProviderEligibility({ ...(providerSnap.data() || {}), _commercialAgreement: agreementSnap.exists ? agreementSnap.data() : null }, existing.exists ? existing.data() : null, storeSnap.data() || {}, shipmentsSnap.docs.length)
...
function normalizeCommercialAgreement(input: any, providerId: string) {
  ...
  for (const tier of tiers) if (tier.maxShipments != null && tier.maxShipments < tier.minShipments) throw new HttpsError('invalid-argument', 'نهاية الشريحة يجب أن تكون أكبر من بدايتها')
  ...
  if (input?.status === 'active' && tiers.length === 0) throw new HttpsError('invalid-argument', 'الاتفاقية النشطة تحتاج شريحة واحدة على الأقل')
  const from = input?.effectiveFrom ? Date.parse(String(input.effectiveFrom)) : null; const to = input?.effectiveTo ? Date.parse(String(input.effectiveTo)) : null
...
// @ts-expect-error OPEN estimates may intentionally contain null revenue when no tier matches.

export const getShippingPartnerRevenuePeriod = onCall(async (request: CallableRequest<any>) => { await assertPlatformAdmin(request); const providerId = String(request.data?.providerId || '').trim(); const period = String(request.data?.period || '').trim(); if (!providerId || !/^\d{4}-\d{2}$/.test(period)) throw new HttpsError('invalid-argument', 'providerId والفترة مطلوبان'); const snap = await db.doc(`shippingPartnerRevenuePeriods/${providerId}_${period}`).get(); if (snap.exists) return { period: { id: snap.id, ...snap.data() } }; const agreement = (await db.doc(`shippingProviderCommercialAgreements/${providerId}`).get()).data() || {}; const start = new Date(`${period}-01T00:00:00Z`); const end = new Date(start); end.setUTCMonth(end.getUTCMonth() + 1); const docs = (await db.collection('shipments').where('providerId', '==', providerId).where('createdAt', '>=', start).where('createdAt', '<', end).get()).docs.map((d) => d.data()); const deliveredCount = docs.filter((d) => ['DELIVERED', 'delivered'].includes(String(d.status || d.currentStatus))).length; const returnedCount = docs.filter((d) => ['RETURNED', 'returned'].includes(String(d.status || d.currentStatus))).length; const pendingCount = Math.max(0, docs.length - deliveredCount - returnedCount); const tier = (agreement.tiers || []).find((t: any) => docs.length >= Number(t.minShipments || 0) && (t.maxShipments == null || docs.length <= Number(t.maxShipments))) || null; const deliveredRevenue = tier ? deliveredCount * Number(tier.deliveredCommission || 0) : null; const returnedRevenue = tier ? returnedCount * Number(tier.returnedCommission || 0) : null; return { period: { providerId, period, shipmentVolume: docs.length, deliveredCount, returnedCount, pendingCount, tierSnapshot: tier, estimated: true, deliveredRevenue, returnedRevenue, totalRevenue: deliveredRevenue == null ? null : deliveredRevenue + returnedRevenue, status: 'OPEN' } } })

export const finalizeShippingPartnerRevenuePeriod = onCall(async (request: CallableRequest<any>) => { await assertPlatformAdmin(request); const providerId = String(request.data?.providerId || '').trim(); const period = String(request.data?.period || '').trim(); if (!providerId || !/^\d{4}-\d{2}$/.test(period)) throw new HttpsError('invalid-argument', 'providerId والفترة مطلوبان'); const ref = db.doc(`shippingPartnerRevenuePeriods/${providerId}_${period}`); const current = await ref.get(); if (current.exists && ['FINALIZED', 'SETTLED'].includes(current.data()?.status)) return { period: { id: ref.id, ...current.data() } }; const agreement = (await db.doc(`shippingProviderCommercialAgreements/${providerId}`).get()).data(); if (!agreement || agreement.status !== 'active' || !Array.isArray(agreement.tiers) || agreement.tiers.length === 0) throw new HttpsError('failed-precondition', 'لا توجد اتفاقية تجارية نشطة'); const start = new Date(`${period}-01T00:00:00Z`); const end = new Date(start); end.setUTCMonth(end.getUTCMonth() + 1); const from = agreement.effectiveFrom ? toMillis(agreement.effectiveFrom) : null; const to = agreement.effectiveTo ? toMillis(agreement.effectiveTo) : null; if ((from != null && start.getTime() < from) || (to != null && end.getTime() > to)) throw new HttpsError('failed-precondition', 'الفترة خارج مدة الاتفاقية'); const docs = (await db.collection('shipments').where('providerId', '==', providerId).where('createdAt', '>=', start).where('createdAt', '<', end).get()).docs.map((d) => d.data()); const deliveredCount = docs.filter((d) => ['DELIVERED', 'delivered'].includes(String(d.status || d.currentStatus))).length; const returnedCount = docs.filter((d) => ['RETURNED', 'returned'].includes(String(d.status || d.currentStatus))).length; const pendingCount = Math.max(0, docs.length - deliveredCount - returnedCount); const tier = agreement.tiers.find((t: any) => docs.length >= Number(t.minShipments || 0) && (t.maxShipments == null || docs.length <= Number(t.maxShipments))); if (!tier) throw new HttpsError('failed-precondition', 'لا توجد شريحة مطابقة لحجم الشحنات'); const data = { providerId, period, shipmentVolume: docs.length, deliveredCount, returnedCount, pendingCount, tierSnapshot: tier, deliveredRevenue: deliveredCount * Number(tier.deliveredCommission || 0), returnedRevenue: returnedCount * Number(tier.returnedCommission || 0), totalRevenue: deliveredCount * Number(tier.deliveredCommission || 0) + returnedCount * Number(tier.returnedCommission || 0), status: 'FINALIZED', finalizedAt: now(), agreementVersion: String(agreement.updatedAt?.seconds || '') }; await ref.set({ ...data, createdAt: current.exists ? current.data()?.createdAt : now() }, { merge: false }); return { period: { id: ref.id, ...data } } })

export const markShippingPartnerRevenueSettled = onCall(async (request: CallableRequest<any>) => { await assertPlatformAdmin(request); const providerId = String(request.data?.providerId || '').trim(); const period = String(request.data?.period || '').trim(); const ref = db.doc(`shippingPartnerRevenuePeriods/${providerId}_${period}`); const snap = await ref.get(); if (!snap.exists || snap.data()?.status !== 'FINALIZED') throw new HttpsError('failed-precondition', 'لا يمكن تسوية فترة غير نهائية'); const data = { status: 'SETTLED', settledAt: now(), settledBy: request.auth!.uid, settlementReference: request.data?.settlementReference ? sanitizeSensitiveText(String(request.data.settlementReference)).slice(0, 200) : null, settlementNotes: request.data?.settlementNotes ? sanitizeSensitiveText(String(request.data.settlementNotes)).slice(0, 1000) : null }; await ref.update(data); return { period: { id: ref.id, ...snap.data(), ...data } } })
