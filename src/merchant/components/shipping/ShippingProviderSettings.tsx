...
const standardDraft = (provider: ShippingProviderDefinition, config: StoreShippingProviderConfig) => ({
  fixedRate: String(config.fixedRate ?? ''), defaultPackageWeight: String(config.defaultPackageWeight ?? ''), codEnabled: config.codEnabled !== false,
  serviceCode: config.serviceCode || provider.services?.[0]?.code || '', rateMarkup: String(config.rateMarkup ?? ''), freeShippingThreshold: String(config.freeShippingThreshold ?? ''),
})
...
const WaslaProviderSettings: FunctionalComponent<Props> = ({ storeId, provider, config, saving, onSave, onSaveCredentials }) => {
  ...
  const [draft, setDraft] = useState({
    locationName: String(pickup.locationName ?? config.waslaPickupLocationName ?? ''), contactPhone: String(pickup.contactPhone ?? config.waslaPickupContactPhone ?? ''),
    addressLine1: String(pickup.addressLine1 ?? config.waslaPickupAddressLine1 ?? ''), governorateId: String(pickup.governorateId ?? config.waslaPickupGovernorateId ?? ''), cityId: String(pickup.cityId ?? config.waslaPickupCityId ?? ''), apiKey: '',
  })
  ...
  return <div className="shipping-provider-config grid grid-2 mt-1">
    ...
    <Button size="sm" className="shipping-provider-save" loading={saving} onClick={() => onSave({ serviceCode: config.serviceCode || provider.services?.[0]?.code || '', enabledServiceCodes: config.serviceCode ? [config.serviceCode] : [], defaultPackageWeight: Number(config.defaultPackageWeight || 0), codEnabled: config.codEnabled !== false, rateMode: 'api', providerConfig: { wasla: { pickup: { locationType: 'merchant_store', locationName: draft.locationName, contactPhone: draft.contactPhone, addressLine1: draft.addressLine1, governorateId: Number(draft.governorateId || 0), cityId: Number(draft.cityId || 0) } } } })}>حفظ إعدادات الشركة</Button>
  </div>
}
...
const GenericProviderSettings: FunctionalComponent<Props> = ({ provider, config, saving, onSave, onSaveCredentials }) => {
  ...
  return <div className="shipping-provider-config grid grid-2 mt-1">
    ...
    {provider.integrationType === 'api' && provider.adapterConfigured && provider.slug !== 'bosta' && <div className="grid grid-2" style={{ gridColumn: '1 / -1' }}><Input label="مفتاح API" type="password" value={apiKey} placeholder={config.credential?.maskedCredentials?.apiKey || 'أدخل مفتاح API'} onChange={setApiKey} /><Button size="sm" variant="outline" loading={saving} disabled={!apiKey.trim()} onClick={() => onSaveCredentials({ apiKey })}>حفظ بيانات الربط بأمان</Button></div>}
...
/** The generic merchant page delegates carrier settings to this provider boundary. */
export const ShippingProviderSettings: FunctionalComponent<Props> = (props) => props.provider.slug === 'wasla'
  ? <WaslaProviderSettings {...props} />
  : props.provider.slug === 'bosta'
    ? <BostaProviderSettings {...props} />
  : <GenericProviderSettings {...props} />
