...
export const LandingPage: FunctionalComponent = () => {
  ...
  return <div className="landing" dir="rtl">
    ...
    <main>
      ...
      <section id="pricing" className="landing-section landing-pricing"><div className="landing-container"><div className="landing-section-heading"><span>الأسعار الشهرية</span><h2>شهر مجاني، ثم باقة تناسب تشغيلك</h2><p>Free لمدة 30 يومًا. Starter/Growth/Pro جرّب كل باقة مجانًا لمدة 3 أيام بمزاياها الكاملة. Lifetime منتج منفصل بدون تجربة.</p></div>{publicPromotion && <div className="landing-promotion-banner"><div><strong>عرض خاص على {publicPromotion.planName || 'الباقة المختارة'}</strong><span>سعر ترويجي لفترة محدودة</span></div>{publicPromotion.endsAt && <CountdownTimer endsAt={publicPromotion.endsAt} label="ينتهي خلال" />}</div>}<div className="landing-pricing-grid stitch-pricing-grid">{subscriptionPlans.map((plan) => {
