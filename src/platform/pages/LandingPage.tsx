...
export const LandingPage: FunctionalComponent = () => {
  ...
  return <div className="landing" dir="rtl">
    ...
    <main>
      ...
      <section id="shipping-partners" className="landing-section landing-shipping-partners"><div className="landing-container"><div className="landing-section-heading"><span>شركاء الشحن والتوصيل</span><h2>شركاؤك في توصيل الطلبات</h2><p>اربط متجرك بخدمات الشحن المتاحة على متجري وأدر دورة الطلب من مكان واحد.</p></div>{shippingPartners.length > 0 && <div className="landing-partners-grid">{shippingPartners.map((partner) => <a className="landing-partner-card" key={partner.id} href={partner.websiteUrl || '#'} target={partner.websiteUrl ? '_blank' : undefined} rel={partner.websiteUrl ? 'noopener noreferrer' : undefined}><img src={partner.logoUrl} alt={partner.name} /><span>{partner.name}</span></a>)}</div>}<div className="landing-partner-cta"><strong>هل تمثل شركة شحن؟</strong><Link href="/partners/shipping/apply">انضم كشريك شحن</Link></div></div></section>
