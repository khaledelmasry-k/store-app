...
const FAQS = [
  ...
  ['هل أستطيع الترقية لاحقاً؟', 'نعم. يمكنك اختيار الباقة المناسبة خلال الشهر المجاني أو بعد انتهائه من صفحة الاشتراك.'],
...
export const LandingPage: FunctionalComponent = () => {
  ...
  return <div className="landing" dir="rtl">
    ...
    <main>
      ...
      <section id="pricing" className="landing-section landing-pricing"><div className="landing-container"><div className="landing-section-heading"><span>الأسعار الشهرية</span><h2>شهر مجاني، ثم باقة تناسب تشغيلك</h2><p>Free لمدة 30 يومًا وليست مجانية للأبد. بعدها اختر Starter أو Growth أو Pro. لا تُحذف بياناتك عند انتهاء التجربة.</p></div>{publicPromotion && <div className="landing-promotion-banner"><div><strong>عرض خاص على {publicPromotion.planName || 'الباقة المختارة'}</strong><span>سعر ترويجي لفترة محدودة</span></div>{publicPromotion.endsAt && <CountdownTimer endsAt={publicPromotion.endsAt} label="ينتهي خلال" />}</div>}<div className="landing-pricing-grid stitch-pricing-grid">{subscriptionPlans.map((plan) => <div className="landing-plan-wrap stitch-plan-wrap" key={plan.id}><PricingCard plan={plan} featured={!!plan.isPopular} /><Link href={`/register?plan=${plan.id}`} className="landing-plan-link stitch-plan-link">{plan.id === 'plan-free' ? 'ابدأ شهرك المجاني' : 'ابدأ Free ثم اختر الباقة'}</Link></div>)}</div>{lifetimeOffer && <section className="landing-special-offer stitch-lifetime-offer"><div><span>منتج منفصل عن الاشتراكات</span><h2>امتلك متجرك مع Lifetime Access</h2><p>حق استخدام دائم لمتجر واحد وفق حدود العرض المحددة، عبر طلب شراء ومراجعة دفع منفصلين.</p><Link href="/register?offer=lifetime" className="landing-special-cta stitch-offer-cta">اعرف التفاصيل</Link></div><PricingCard plan={lifetimeOffer} displayName="Lifetime Access" /></section>}</div></section>
