...
const ShippingRevenue: FunctionalComponent = () => {
  ...
  const load = async () => { setLoading(true); try { const providers = await listDocs<any>('shippingProviders', { where: { status: { value: 'active' } } }); setHasProviders(providers.length > 0); const next = []; for (const p of providers) { if (providerFilter && p.id !== providerFilter) continue; try { const agreement: any = await getShippingProviderCommercialAgreementCallable({ providerId: p.id }); const revenue: any = await getShippingPartnerRevenuePeriodCallable({ providerId: p.id, period: month }); next.push({ provider: p, agreement: agreement.data?.agreement, period: revenue.data?.period }) } catch { next.push({ provider: p, agreement: null, period: null }) } } setRows(next) } finally { setLoading(false); setInitialLoading(false) } }
