...
export const PlatformCrm: FunctionalComponent = () => {
  ...
  return <div className="platform-operations">
    ...
    <Card title="التجار"><div className="filter-bar"><Input label="بحث" value={query} onChange={setQuery} placeholder="اسم المتجر أو البريد" /><Select label="المرحلة" value={stage} onChange={setStage} options={[{ value: '', label: 'كل المراحل' }, ...STAGES.map((s) => ({ value: s, label: s }))]} /><Select label="الخطة" value={plan} onChange={setPlan} options={[{ value: '', label: 'كل الخطط' }, ...Array.from(new Set(merchants.map((m) => m.plan).filter(Boolean))).map((p) => ({ value: p, label: p }))]} /><Select label="حالة الاشتراك" value={subscriptionStatus} onChange={setSubscriptionStatus} options={[{ value: '', label: 'كل الحالات' }, ...Array.from(new Set(merchants.map((m) => m.subscriptionStatus).filter(Boolean))).map((s) => ({ value: s, label: s }))]} /></div>{filtered.length === 0 ? <EmptyState title="لا يوجد تجار" description="غيّر الفلاتر أو أضف تاجرًا جديدًا." /> : <div className="table-wrap"><table><thead><tr><th>المتجر</th><th>المالك</th><th>الخطة</th><th>الحالة</th><th>المرحلة</th><th /></tr></thead><tbody>{filtered.map((m) => <tr key={m.id}><td>{m.storeName}</td><td>{m.ownerName || m.email}</td><td>{m.plan || '—'}</td><td>{m.subscriptionStatus || '—'}</td><td>{m.stage || '—'}</td><td><Button size="sm" variant="ghost" onClick={() => void openMerchant(m)}>360</Button></td></tr>)}</tbody></table></div>}</Card>
